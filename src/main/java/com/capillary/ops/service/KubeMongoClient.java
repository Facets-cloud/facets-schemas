...
public class KubeMongoClient {
  ...
  private void constructMongoCredentialsAndOptions() {
    ...
    mongoClientOptions = MongoClientOptions.builder().sslEnabled(false).build();
  }
  ...
  public Document execute(BasicDBObject command) {
    MongoClient mongoClient =
        new MongoClient(
            new ServerAddress(this.deploymentName + "-mongodb.infra", this.port),
            mongoCredential,
            mongoClientOptions);
  ...
  public Map<Integer, String> execute(List<BasicDBObject> commands) {
    ...
    for (int commandStatusCounter = 1;
        commandStatusCounter <= commands.size();
  ...
  public boolean isExecuteFunctionPermissionPresent() {
    ...
    Document document =
        mongoClient.getDatabase("admin").getCollection("system.roles").find(command).first();
    mongoClient.close();
  ...
  public void createExecuteFunctionRoleAndGivePermission() {
    ...
    PodList podList =
        kubernetesClient
            .pods()
            .inNamespace("infra")
            .withLabel("release", this.deploymentName)
            .list();
    podList
        .getItems()
        .forEach(
            pod -> {
              ...
              String createExecuteFunctionsRole =
                  String.format(MONGO_EVAL_COMMAND, this.password, CREATE_EXECUTE_FUNCTIONS_ROLE);
              String grantRoleToRootUser =
                  String.format(
                      MONGO_EVAL_COMMAND, this.password, GRANT_EXECUTE_FUNCTIONS_ROLE_TO_ROOT);
  ...
  //
  //        Document document =
  // mongoClient.getDatabase("admin").getCollection("system.users").find(command).first();

...
sub _diff_tables {
    ...
    my @ret;
    if( scalar(@changes) > 0 ) {
        push @ret, $self->_alter_table_clause(@_);
        push ( @ret ,join("\n,", @changes));
    }
    return @ret;
}

sub _alter_table_clause{
...
sub _diff_fields {
    ...
    $fields1 =~ s/KEY_BLOCK_SIZE=[0-9]*/ /gi;
    $fields2 =~ s/KEY_BLOCK_SIZE=[0-9]*/ /gi;
...
sub _diff_primary_key {
    ...
    if (! $primary1 && $primary2) {
        ...
        return ("ADD PRIMARY KEY $primary2");
    }
    ...
    if ($primary1 ne $primary2) {
        ...
        $changes = "ADD PRIMARY KEY $primary2";
...
sub _diff_foreign_key {
        ...
        for my $fk (keys %$fks1) {
            ...
            if ($fks2 && $fks2->{$fk}) {
                if($fks1->{$fk} ne $fks2->{$fk}) 
                {
                    ...
                    my $changes = "DROP FOREIGN KEY $fk";
                    $changes .= " # was CONSTRAINT $fk $fks1->{$fk}"
                        unless $self->{opts}{'no-old-defs'};
                    push @changes, $changes;

                    $changes = "$name1 ADD CONSTRAINT $fk FOREIGN KEY $fks2->{$fk},";
                    push @changes, $changes;
                }
            } else {
    ...
    if($fks2) {
        for my $fk (keys %$fks2) {
            ...
            push @changes, "ADD CONSTRAINT $fk FOREIGN KEY $fks2->{$fk}";
        }
    }
...
sub _check_for_auto_col {
    ...
    for my $field (@fields) {
        next if(defined($table->field($field)) && $table->field($field) !~ /auto_increment/i);
